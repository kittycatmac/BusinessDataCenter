...
jest.mock('../Data/getShipJobData', () => ({
    getShipJobData: jest.fn(),
}));
...
test('EditShipSurvey displays data', async () => {
    ...
    //await screen.findByText('STUDENT INSIDER FALL 2023 NAU EDITION', { visible: true });
    await screen.findByText('Ship Id', { visible: true });
    await screen.findByText('77575', { visible: true });
    //await screen.findByText('Quanity', { visible: true });
    await screen.findByText('22000', { visible: true });
    //await screen.findByText('Shipment Value', { visible: true });
    //await screen.findByText('$11,145.04', { visible: true });
    await screen.findByText('Ship Date', { visible: true });
    await screen.findByText('8/15/2023', { visible: true });
    //await screen.findByText('OnTime Completed', { visible: true });
    //await screen.findByText('Done', { visible: true });
